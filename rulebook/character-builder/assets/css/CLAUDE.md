...
The web page (`character-builder.html`) **MUST** only load a single, combined stylesheet: `character-builder.css`. This is a non-negotiable performance requirement.
...
The modular CSS files in the `base/`, `components/`, and `tabs/` directories (e.g., `_buttons.css`, `_cards.css`) are for **developer organization only**.
...
The `character-builder.css` file is a **generated file**. It is not meant to be edited directly.

-   The PowerShell script acts as our "build tool."
-   Its only job is to **read** all the small partial files and **concatenate** (combine) them, in the correct order, into the single `character-builder.css` file.
-   Any manual edits made to `character-builder.css` will be **deleted** the next time the build script is run.
...
1.  **Identify the Correct Partial File.**
    -   Want to change a button's color? Open `css/components/_buttons.css`.
    -   Want to change the layout of the attributes tab? Open `css/tabs/_attributes.css`.
    -   Want to change a global color variable? Open `css/base/_variables.css`.
...
3.  **Run the Build Script.**
    ...
    -   Run the final concatenation script (v4 or later) to rebuild the main `character-builder.css` file.
...
-   **NEVER** edit `rulebook/character-builder/css/character-builder.css` by hand.
