...
# The full path to the file that needs to be updated
file_path = Path("frontend/campaigns/rogue_trader/index.html")
...
<body class="theme-warrant">
    <div class="warrant-container">
        ...
        <main class="content" id="main-content-area">
            ...
            <div class="tab-container">
                <ul class="tab-nav">
                    <li><button class="tab-button active" data-tab="briefing">Briefing</button></li>
                    <li><button class="tab-button" data-tab="lore">Lore Archives</button></li>
                    <li><button class="tab-button" data-tab="logbook">Campaign Log</button></li>
                </ul>
            </div>
    ...
    <script>
        class RogueTraderCampaignApp {
                ...
                this.tabConfig = {
                    ...
                    'logbook': {
                        ...
                        docs: {
                            ...
                            'notes': { label: 'Session Notes', path: 'campaign_log/session-notes.md' }
                        }
                    }
                };
            ...
            // Renders the sidebar based on the active main tab
            renderSidebar() {
                const navContent = document.getElementById('nav-content');
                if (!navContent) return;

                const currentTabDocs = this.tabConfig[this.activeMainTab].docs;
                let navHTML = Object.entries(currentTabDocs).map(([key, doc]) => `
                    <a href="#" class="nav-item h2 ${key === this.activeDocKey ? 'active' : ''}" 
                       data-action="select-doc" data-doc-key="${key}">
                        ${doc.label}
                    </a>
                `).join('');
            ...
            async handleMainTabSwitch(newTabKey) {
                if (newTabKey === this.activeMainTab) return;

                this.activeMainTab = newTabKey;
                this.activeDocKey = this.tabConfig[newTabKey].defaultDoc;

                // Update active class on main tabs
                document.querySelectorAll('.tab-button').forEach(btn => {
                    btn.classList.toggle('active', btn.dataset.tab === newTabKey);
                });

                this.renderSidebar();
                const defaultDocPath = this.tabConfig[newTabKey].docs[this.activeDocKey].path;
                await this.loadDocument(defaultDocPath);
            }

            async handleDocSwitch(docKey) {
                if (docKey === this.activeDocKey) return;
                
                this.activeDocKey = docKey;
                
                // Update active class in sidebar
                document.querySelectorAll('#nav-content .nav-item').forEach(item => {
                    item.classList.toggle('active', item.dataset.docKey === docKey);
                });
                
                const docPath = this.tabConfig[this.activeMainTab].docs[docKey].path;
                await this.loadDocument(docPath);
            }

            setupEventListeners() {
                document.addEventListener('click', (e) => {
                    const mainTabButton = e.target.closest('.tab-button');
                    const docLink = e.target.closest('.nav-item[data-action="select-doc"]');
                    const menuToggle = e.target.closest('.menu-toggle');

                    if (mainTabButton) {
                        e.preventDefault();
                        this.handleMainTabSwitch(mainTabButton.dataset.tab);
                    } else if (docLink) {
                        e.preventDefault();
                        this.handleDocSwitch(docLink.dataset.docKey);
                    } else if (menuToggle) {
...
try:
    ...
    file_path.write_text(new_html_content, encoding='utf-8')
    
    print(f"✅ Successfully updated '{file_path}' to use the new directory structure.")
    print("➡️ The page now supports dynamic sub-navigation based on the selected main tab.")
